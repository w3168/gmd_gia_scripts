...
name = f"submesh-withurfDG-3d-internalvariable-dispvel-{args.optional_name}-1dvisc{args.radial_visc}_layered"

full_mesh = Mesh("unstructured_sphere_internal_test_old.msh")
full_mesh.cartesian = False
DG0_full = FunctionSpace(full_mesh, "DG", 0)
mantle_id = 101
exterior_grav_id = 102
# Heaviside step function in mantle
I_mantle = Function(DG0_full)
...
V_grav_DG = FunctionSpace(full_mesh, "DG", 2)  # Displacement function space (vector)
V_grav_mantle = FunctionSpace(mesh, "CG", 2)  # Displacement function space (vector)
V_grav_mantle_DG = FunctionSpace(mesh, "DG", 2)  # Displacement function space (vector)
...
DG1_grav = FunctionSpace(full_mesh, "DG", 1)  # (Discontinuous) Stress tensor function space (tensor)
DG1_vec = VectorFunctionSpace(mesh, "DG", 1)  # (Discontinuous) Stress tensor function space (tensor)
DG2_vec = VectorFunctionSpace(mesh, "DG", 2)  # (Discontinuous) Stress tensor function space (tensor)
DG2_vec_grav = VectorFunctionSpace(full_mesh, "DG", 2)  # (Discontinuous) Stress tensor function space (tensor)
CG2_vec_grav = VectorFunctionSpace(full_mesh, "CG", 2)  # (Discontinuous) Stress tensor function space (tensor)
...
grad_phi = Function(DG1_vec, name='grad phi')
...
ice_load = Vi * rho_ice * (Hice1 * disc1)
...
coupled_solver = InternalVariableSolver(z, approximation, coupled_dt=dt, grad_phi=grad_phi, bcs=stokes_bcs,
...
f = Function(V).interpolate(as_vector([X[0], X[1], X[2]]))
...
if OUTPUT:
    ...
    output_file.write(*z.subfunctions, vertical_displacement, grad_phi)
...
grav_solution_DG = Function(V_grav_DG, name="phi")
grav_solution_mantle = Function(V_grav_mantle, name="phi mantle")
grav_solution_mantle_DG = Function(V_grav_mantle_DG, name="phi mantle")
# For the forcing term we use DG to capture the density jump
#rho1_grav = Function(DG0_grav, name="rho1")

ice_load_func = Function(DG1, name="iceload").interpolate(rho_ice * (Hice1 * disc1))
ice_load_func_grav = Function(DG1_grav, name="iceload").interpolate(ice_load_func, subset=full_mesh.topology.cell_subset(999))

density_grav = Function(DG0_grav, name='rho0')
density_grav.interpolate(density ,subset=full_mesh.topology.cell_subset(999))

z_DG = Function(DG2_vec)
z_DG_grav = Function(DG2_vec_grav)
z_grav = Function(CG2_vec_grav)

# Interpolate disp across to grav mesh via DG funcs
z_DG.interpolate(z.subfunctions[0])

# need to keep disp in DG otherwise artefacts at the Re boundary which blow up!
z_DG_grav.interpolate(z_DG,subset=full_mesh.topology.cell_subset(999))


grav_bcs = [DirichletBC(V_grav, 0.0, Rg_id)]

# outward pointint normal vector to infitiy (coped from FSI demo on firedrake)
n = FacetNormal(full_mesh)

# I think notes I have (and sia's old github had nabla^2 phi = 4 pi G rho (but in poiosson-graviy add minus sign on lhs. taking this out now...)
a = -inner(grad(v), grad(phi)) * dx  

# Free surface term (moved to rhs), one of terms should be zero as density and disp are zero outside
#        F = Vi('+') * jump(rho0) * u_r('+') * g('+') * dot(eq.test('+'), eq.n('+')) * dS
L = - 4 * pi * v('+') * density_grav('+') * dot(z_grav('+'), n('+'))  * dS(2)
L -=  4 * pi * v('-') * density_grav('-') * dot(z_grav('-'), n('-'))  * dS(2)

# internal density interfaces
L -=  4 * pi * v('+') * jump(density_grav) * dot(z_grav('+'), n('+'))  * dS(11)
L -=  4 * pi * v('+') * jump(density_grav) * dot(z_grav('+'), n('+'))  * dS(12)
L -=  4 * pi * v('+') * jump(density_grav) * dot(z_grav('+'), n('+'))  * dS(13)

# ice load term (moved to rhs), one of terms should be zero as ice load is zero outside the mantle
L -=  4 * pi * v('+') * ice_load_func_grav('+')  * dS(2)
L -=  4 * pi * v('-') * ice_load_func_grav('-')  * dS(2)

# RHS rho1 forcing term
rho1_grav = - dot(z_DG_grav, grad(density_grav)) - density_grav*div(z_DG_grav)
...
# convert CG to DG for mesh to mesh interp
grav_solution_DG.interpolate(grav_solution)
# interp grav DG from grav mesh to mantle
grav_solution_mantle_DG.interpolate(grav_solution_DG)
# convert back from DG to CG
grav_solution_mantle.interpolate(grav_solution_mantle_DG)

grad_phi.interpolate(grad(grav_solution_mantle))


if OUTPUT:
    grav_output_file = VTKFile(f"{args.output_path}{name}-unstructured-dt{dt_years}years-bulk{args.bulk_shear_ratio}-nondim-gravity2.pvd")
    grav_output_file.write(grav_solution, z_grav, z_DG_grav, density_grav)

for timestep in range(1, max_timesteps+1):
    # update time first so that ice load begins
    time.assign(time+dt)
    coupled_solver.solve()
#    rho1_grav.interpolate(dot(z.subfunctions[0], grad(density))+density*div(z.subfunctions[0]), allow_missing_dofs=True)
#    rho1_grav.assign(density) #, allow_missing_dofs=True)
    
    # Interpolate disp across to grav mesh via DG funcs
    z_DG.interpolate(z.subfunctions[0])
    z_DG_grav.interpolate(z_DG,subset=full_mesh.topology.cell_subset(999))
    z_grav.interpolate(z_DG_grav)
