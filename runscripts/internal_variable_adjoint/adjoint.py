...
#    minimisation_parameters["Step"]["Trust Region"]["Initial Radius"] = 1e4
    ...
    updated_ice_thickness_file = VTKFile("updated_ice_thickness_rotnull_noreg.pvd")
    updated_out_file = VTKFile("updated_out_rotnull.pvd")
    functional_values = []
...
def ice_taylor_test(): #alpha_T, alpha_u, alpha_d, alpha_s):
    """
...
#        velocity_misfit += assemble(dot(velocity_error, velocity_error) / (circumference * velocity_scale**2) * ds(boundary.top))
    ...
    alpha_smoothing = 0.0
    alpha_damping = 0.0
    damping = assemble((normalised_ice_thickness) ** 2 / circumference * ds(boundary.top))
    smoothing = assemble(dot(grad(normalised_ice_thickness), grad(normalised_ice_thickness)) / circumference * ds(boundary.top))
...
#        updated_velocity.interpolate(z.subfunctions[0].block_variable.checkpoint / dt)
    ...
    inverse_problem["control"] = normalised_ice_thickness

    # The ReducedFunctional that is to be minimised
    inverse_problem["reduced_functional"] = ReducedFunctional(objective, control, eval_cb_post=eval_cb)
