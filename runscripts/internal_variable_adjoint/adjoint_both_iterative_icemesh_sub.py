...
#    checkpoint_file = "displacement-objective-forward-cylinder-2d-internalvariable-dispvel--ncells360.0-nz20perlayer-dt50.0years-bulk1.94-nondim.h5"
    if args.burgers:
        checkpoint_file = "displacement-objective-forward-cylinder-2d-internalvariable-dispvel--1dviscFalse-burgersTrue-ncells360.0-nz20perlayer-dt50.0years-bulk1.94-nondim.h5"
    else:
...
#    gd = GeodynamicalDiagnostics(z, density, boundary.bottom, boundary.top)
    ...
    approximation = CompressibleInternalVariableApproximation(bulk_modulus=bulk_modulus, density=density, shear_modulus=shearmod_list, viscosity=visc_list, Vi=Vi, bulk_shear_ratio=args.bulk_shear_ratio, compressible_buoyancy=True, compressible_adv_hyd_pre=True)
    ...
    nullspace = create_stokes_nullspace(Z, closed=False, rotational=True)
    near_nullspace = create_stokes_nullspace(Z, closed=True, rotational=True, translations=[0, 1])

    solver = InternalVariableSolver(u, approximation, dt=dt, m_list=m_list, bcs=stokes_bcs,
